...
namespace Matcher {
    public class DNAMatcher {
        ...
        public int ComputeSimiliarityWithPenalty(out string[] matching) {
            ...
            for (int i = 1; i <= m; i++)
                S[0, i] = A[0, i] = PenaltyFunction(i);

            for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= m; j++) {
                    int max = int.MinValue;
                    for (int k = 0; k < j; k++)
                        max = Math.Max(max, Math.Max(B[i, k], C[i, k]) + PenaltyFunction(j - k));
                    A[i, j] = max;

                    max = int.MinValue;
                    for (int k = 0; k < i; k++)
                        max = Math.Max(max, Math.Max(A[k, j], C[k, j]) + PenaltyFunction(i - k));
                    B[i, j] = max;

                    C[i, j] = S[i - 1, j - 1] + s[uGlobal[i - 1], wGlobal[j - 1]];

                    S[i, j] = MathUtils.Max(A[i, j], B[i, j], C[i, j]);
                }
            }

            if (Verbose) {
                Console.WriteLine("A");
                PrintMatrix(uGlobal, wGlobal, A);
                Console.WriteLine("B");
                PrintMatrix(uGlobal, wGlobal, B);
                Console.WriteLine("C");
                PrintMatrix(uGlobal, wGlobal, C);
                Console.WriteLine("S");
        ...
        public int Hirschberg(byte[] u, byte[] w, out string[] matching) {
            ...
            if (u.Length == 0) {
                ...
                for (int i = 0; i < w.Length; i++) {
                    matchingBuilder1.Append('_');
                    matchingBuilder2.Append(ByteToDNA(w[i]));
                    cost += s[DNAToByte('_'), w[i]];
                }
                ...
                for (int i = 0; i < u.Length; i++) {
                    matchingBuilder1.Append(ByteToDNA(u[i]));
                    matchingBuilder2.Append('_');
                    cost += s[u[i], DNAToByte('_')];
                }
                matching = new string[] { matchingBuilder1.ToString(), matchingBuilder2.ToString() };
                return cost;
            }
